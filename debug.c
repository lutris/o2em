...
void debug(void)
{
    ...
    while (!done) {
        ...
        printf("> ");
        result = fgets(inp, 76, stdin);
        if (result != NULL && strlen(inp) > 0 && inp[strlen(inp) - 1] == '\n')
            inp[strlen(inp) - 1] = 0;
        strlwr(inp);
        strcat(inp, " \n");
        tok = strtok(inp, " ");
...
//			printf("editsprite # - edit memory # as sprite\n");
            ...
            printf(
                    "video (a d) - display VDC registers (or write data d to a)\n");
            printf(
                    "extram (a d) - display external RAM (or write data d to a)\n");
            printf(
                    "intram (a d) - display internal ram (or write data d to a)\n");
...
//----------------------------------------------------------------------------------------------------
                ...
                if (strlen(file) > 0) {
                    ...
                    if (tok) {
                        ...
                        if (tok) {
                            ...
                            if (fn == NULL) {
                                fprintf(stderr,
                                        "Error opening save-file %s: %i\n",
                                        file, errno);
                            } else {
                                fwrite((rom + adr), t, 1, fn);
                                fclose(fn);
                                printf(
                                        "File %s saved: %i bytes from address:%x\n",
                                        file, t, adr);
                            }
                        }
                    }
                ...
                if (strlen(file) > 0) {
                    ...
                    if (tok) {
                        ...
                        if (tok) {
                            ...
                            if (fn == NULL) {
                                fprintf(stderr,
                                        "Error opening save-data file %s: %i\n",
                                        file, errno);
                            } else {
                                // write format "db 0XXh[,0XXh]" 8 Zeichen pro Zeile
                                int j = 0;
                                for (i = 0; i < ((int) ((t - 1) / 8)) + 1;
                                        i++) {
                                    fprintf(fn, "db	");
                    ...
                    if (tok) {
                        ...
                        if (tok) {
                            ...
                            if (fn == NULL) {
                                ...
                                for (i = 0; i < t; i++) {
                                    decimal2binary(rom[adr + i], binary_value,
                                            8);
                                    fprintf(fn, "db	%sb\n", binary_value);
                                }
                        ...
                        for (i = 0; i < 25; i++) {
                            ...
                            for (j = 0; j < 40; j++) {
                                if (fwrite(&vpp_mem[j][i][0], 1, 1, fn) != 1) {
                                    fprintf(
                                            stderr,
                                            "Error writing vpp data file %s: %i\n",
                                            file, errno);
                                    break;
                                }
            ...
            if (app_data.sound_en) {
                ...
                printf("Sound emulation disabled\n\n");
...
static int assemble(char *opcode, int p)
{
    ...
    while ((op < 256)
            && (strncasecmp(opcode, lookup[op].mnemonic,
                    strlen(lookup[op].mnemonic)) != 0))
        op++;
